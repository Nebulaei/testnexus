...
public class RedisConfiguration extends CachingConfigurerSupport {
    ...
    public CacheManager cacheManager() {
        ...
        builders.put("shortTermCache", Caffeine.newBuilder()
                .expireAfterWrite(10, TimeUnit.MINUTES)
                .maximumSize(100));

        // 通过自定义CacheLoader实现不同缓存的不同配置
        cacheManager.setCacheLoader(cacheName -> {
            Caffeine<Object, Object> builder = builders.getOrDefault(cacheName,
                    Caffeine.newBuilder()
                            .initialCapacity(100)
                            .maximumSize(1000)
                            .expireAfterWrite(2, TimeUnit.HOURS));
        ...
        // 预先初始化这些缓存
        cacheManager.setCacheNames(builders.keySet());
        return cacheManager;
    }
